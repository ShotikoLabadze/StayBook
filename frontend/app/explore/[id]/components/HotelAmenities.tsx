...
export default function HotelAmenities({ hotel }: HotelAmenitiesProps) {
  ...
  return (
    <div className="bg-white border border-slate-100 p-7 rounded-3xl shadow-2xs space-y-5 text-left">
      ...
      <div className="flex flex-wrap gap-2.5">
        {amenitiesList.map((amenity, idx) => (
          <div
            key={idx}
            className="flex items-center gap-2.5 px-4 py-2.5 bg-slate-50 border border-slate-100 rounded-xl text-xs font-semibold text-slate-600 hover:bg-slate-100/70 hover:border-slate-200 transition-all duration-200"
          >
