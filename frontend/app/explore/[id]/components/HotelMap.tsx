...
export default function HotelMap({ hotel }: HotelMapProps) {
  ...
  const customIcon = L.divIcon({
    ...
    html: `
      <div class="flex items-center justify-center w-8 h-8 bg-primary rounded-full border-2 border-white shadow-md text-secondary animate-bounce">
  ...
  return (
    <div className="bg-white border border-slate-200/60 p-7 rounded-3xl shadow-2xs space-y-5 text-left">
      ...
      <div className="w-full h-[280px] rounded-2xl overflow-hidden border border-slate-100 shadow-3xs relative z-10">
        <MapContainer
          center={position}
          zoom={14}
        ...
        >
          ...
          <Marker position={position} icon={customIcon}>
            <Popup>
              <div className="font-body text-xs font-bold text-primary p-0.5">
                {hotel.name}
              </div>
            </Popup>
          </Marker>
